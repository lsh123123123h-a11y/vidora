...
import { getModeReferenceLimits } from "@/utils/videoGenerationPolicy";
...
const mixedClipMediaTypes = computed<ClipMediaType[]>(() => {
  const limits = getModeReferenceLimits(props.mode as string);
  return (Object.keys(limits) as ClipMediaType[]).filter((type) => (limits[type] ?? 0) > 0);
});
  ...
  const dlg = DialogPlugin.confirm({
    ...
    onConfirm: async () => {
      ...
      if (slot === "start" || slot === "end") {
        ...
        const combined = [...imageList.value, ...newItems, ...(data ?? [])];
        const limits = getModeReferenceLimits(props.mode as string);
        const accepted: UploadItem[] = [];
        for (const item of combined) {
          const type = (item.fileType || getFileTypeByExt(item.src)) as ClipMediaType;
          const limit = limits[type];
          const currentCount = accepted.filter((candidate) => (candidate.fileType || getFileTypeByExt(candidate.src)) === type).length;
          if (limit == null || currentCount < limit) accepted.push(item);
        }
        imageList.value = accepted;
      }
    },
    onCancel: () => {
      dlg.destroy();
