...
async function handleGenerate() {
  ...
  try {
    ...
    if (data?.effectivePrompt) props.formData.prompt = data.effectivePrompt;
  ...
  (newVal) => {
    if (newVal) {
      ...
      if (props.formData.id != null) {
        axios
          .post("/assetsGenerate/getEffectivePrompt", { projectId: project.value?.id, assetsId: props.formData.id })
          .then(({ data }) => {
            if (data?.prompt) props.formData.prompt = data.prompt;
          });
      }
      fetchGeneratedImages();
    }
