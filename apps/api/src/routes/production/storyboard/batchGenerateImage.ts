...
import { selectStoryboardsForImageGeneration } from "@/lib/storyboardGenerationSelection";
import { getRootRolesUsedForProduction } from "@/lib/characterGenerationPolicy";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const generateList = selectStoryboardsForImageGeneration(storyboardData, compulsory);
    if (!generateList.length) return res.status(400).send(error("No selected storyboards are marked for image generation."));
    const storyboardAssetRows = await u
      .db("o_assets2Storyboard")
      .leftJoin("o_assets", "o_assets2Storyboard.assetId", "o_assets.id")
      .whereIn(
        "o_assets2Storyboard.storyboardId",
        storyboardData.map((storyboard) => storyboard.id).filter((id): id is number => id != null),
      )
      .select("o_assets.name", "o_assets.type", "o_assets.assetsId");
    const invalidRootRoles = getRootRolesUsedForProduction(storyboardAssetRows);
    if (invalidRootRoles.length) {
      return res.status(400).send(error(`Storyboards must reference a role derivative, not a base role: ${invalidRootRoles.join(", ")}.`));
    }
    const generationStoryboardIds = generateList.map((storyboard) => storyboard.id).filter((id): id is number => id != null);
    const referenceRows = await u
      .db("o_assets2Storyboard")
      .leftJoin("o_assets", "o_assets2Storyboard.assetId", "o_assets.id")
      .leftJoin("o_image", "o_assets.imageId", "o_image.id")
      .whereIn("o_assets2Storyboard.storyboardId", generationStoryboardIds)
      .select(
        "o_assets2Storyboard.storyboardId",
    ...
    const unavailableReferences = referenceRows.filter((row) => row.imageState !== "已完成" || !row.filePath);
    if (unavailableReferences.length) {
      const names = [...new Set(unavailableReferences.map((row) => row.name || `资产 ${row.assetId}`))];
      return res.status(400).send(error(`以下分镜参考资产没有已完成的选中图片：${names.join("、")}。`));
    }
    ...
    imageIds.map(async (id) => {
      ...
      if (filePath) {
        try {
          ...
          throw new Error(`读取参考资产图片失败，图片ID: ${id}。${u.error(e).message}`);
        }
      }
      throw new Error(`参考资产图片不存在，图片ID: ${id}。`);
    }),
  );
  return imageUrls.map((url) => ({ type: "image" as const, base64: url }));
}
