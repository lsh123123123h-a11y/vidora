<template>
  <div class="modelServe">
        ...
        <t-form :data="currentVendor" labelAlign="top">
          ...
          <div class="jb ac">
            ...
            <div style="display: flex; gap: 8px">
              <t-button v-if="canImportCurrentVendorModels" variant="outline" size="small" @click="openCurrentVendorModelImport">
    ...
    <t-dialog
      v-model:visible="currentVendorImportVisible"
      width="600px"
      placement="center"
      header="拉取模型列表"
      :confirmBtn="{ content: '导入所选模型', loading: currentVendorImportSaving }"
      :confirmDisabled="!currentVendorImportModels.some((model) => model.selected)"
      @confirm="importCurrentVendorModels">
      <t-alert theme="info" style="margin-bottom: 16px">
        将使用当前供应商已填写的请求地址和 API Key 读取 <code>/models</code>。请勾选需要的模型并指定用途。
      </t-alert>
      <div style="display: flex; justify-content: flex-end; margin-bottom: 12px">
        <t-button theme="primary" variant="outline" :loading="currentVendorImportLoading" @click="fetchCurrentVendorModels">
          <template #icon><t-icon name="refresh" /></template>
          重新拉取
        </t-button>
      </div>
      <div v-if="currentVendorImportModels.length" class="compatibleModelList">
        <div v-for="model in currentVendorImportModels" :key="model.modelName" class="compatibleModelRow">
          <t-checkbox v-model="model.selected">{{ model.modelName }}</t-checkbox>
          <t-select v-if="model.selected" v-model="model.type" style="width: 120px">
            <t-option value="text">文本</t-option>
            <t-option value="image">图像</t-option>
            <t-option value="video">视频</t-option>
          </t-select>
        </div>
      </div>
      <t-empty v-else description="尚未拉取到可导入的模型" />
    </t-dialog>
    ...
    <t-dialog
      ...
      <div class="data">
        <t-radio-group variant="default-filled" v-model="addMode">
          <t-radio-button value="compatibleAdd">兼容中转站</t-radio-button>
        ...
        <div v-if="addMode === 'compatibleAdd'" class="compatibleAdd">
          <t-alert theme="info" style="margin: 16px 0">
            填写 OpenAI 兼容中转站的地址和 Key 后，可从
            <code>/models</code>
            自动拉取模型。请为导入的模型指定用途；通用协议默认支持文本调用，图像和视频生成需要中转站提供对应的适配接口。
          </t-alert>
          <t-form labelAlign="top">
            <t-form-item label="供应商名称">
              <t-input v-model="compatibleForm.name" placeholder="例如：快跑 AI" clearable />
            </t-form-item>
            <t-form-item label="Base URL">
              <t-input v-model="compatibleForm.baseUrl" placeholder="https://api.example.com/v1" clearable />
            </t-form-item>
            <t-form-item label="API Key">
              <t-input v-model="compatibleForm.apiKey" type="password" placeholder="sk-..." clearable />
            </t-form-item>
          </t-form>
          <div style="display: flex; justify-content: flex-end; margin-bottom: 12px">
            <t-button
              ...
              :disabled="!compatibleForm.baseUrl.trim() || !compatibleForm.apiKey.trim()"
              @click="fetchCompatibleModels">
              <template #icon><t-icon name="refresh" /></template>
              拉取模型列表
            </t-button>
          </div>
          <div v-if="compatibleModels.length" class="compatibleModelList">
            <div v-for="model in compatibleModels" :key="model.modelName" class="compatibleModelRow">
              <t-checkbox v-model="model.selected">{{ model.modelName }}</t-checkbox>
              <t-select v-if="model.selected" v-model="model.type" style="width: 120px">
                <t-option value="text">文本</t-option>
                <t-option value="image">图像</t-option>
          ...
          <div style="display: flex; justify-content: flex-end; margin-top: 16px">
            <t-button
              theme="primary"
              :loading="compatibleSaving"
              :disabled="!compatibleForm.name.trim() || !compatibleModels.some((model) => model.selected)"
              @click="importCompatibleProvider">
              导入所选模型
            </t-button>
          </div>
        </div>
...
const canImportCurrentVendorModels = computed(() => {
  const inputKeys = currentVendor.value?.inputs?.map((input) => input.key) ?? [];
  return inputKeys.includes("apiKey") && inputKeys.includes("baseUrl");
});
...
function handleAddVendor() {
  ...
  compatibleForm.value = { name: "", baseUrl: "", apiKey: "" };
...
type CompatibleModelType = "text" | "image" | "video";
interface CompatibleModelItem {
  modelName: string;
  selected: boolean;
  type: CompatibleModelType;
}
const compatibleForm = ref({ name: "", baseUrl: "", apiKey: "" });
const compatibleModels = ref<CompatibleModelItem[]>([]);
const compatibleLoading = ref(false);
const compatibleSaving = ref(false);
const currentVendorImportVisible = ref(false);
const currentVendorImportModels = ref<CompatibleModelItem[]>([]);
const currentVendorImportLoading = ref(false);
const currentVendorImportSaving = ref(false);

function createCompatibleVendorCode(vendorId: string, name: string) {
  const vendor = {
    id: vendorId,
    version: "2.0",
    author: "Vidora",
    ...
    inputValues: { apiKey: compatibleForm.value.apiKey, baseUrl: compatibleForm.value.baseUrl },
    models: [],
  };
  return `const vendor = ${JSON.stringify(vendor, null, 2)};
const textRequest = (model: any) => {
...
function createCompatibleVendorId(name: string) {
  const normalized = name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 36);
  return `relay-${normalized || "provider"}-${Date.now().toString(36)}`;
}

function createImportedCompatibleModel(item: CompatibleModelItem): VendorModel {
  if (item.type === "text") {
    return { name: item.modelName, modelName: item.modelName, type: "text", think: false };
  }
  if (item.type === "image") {
    return { name: item.modelName, modelName: item.modelName, type: "image", mode: ["text"] };
  }
  return {
    name: item.modelName,
...
function openCurrentVendorModelImport() {
  const vendor = currentVendor.value;
  if (!vendor) return;
  if (!vendor.inputValues.apiKey?.trim() || !vendor.inputValues.baseUrl?.trim()) {
    window.$message.error("请先填写并保存 API Key 和请求地址");
    return;
  }
  currentVendorImportModels.value = [];
  currentVendorImportVisible.value = true;
  void fetchCurrentVendorModels();
}

async function fetchCurrentVendorModels() {
  const vendor = currentVendor.value;
  if (!vendor) return;
  currentVendorImportLoading.value = true;
  try {
    const response = await axios.post("/setting/vendorConfig/fetchModels", {
      baseUrl: vendor.inputValues.baseUrl,
      apiKey: vendor.inputValues.apiKey,
    });
    const existingModelNames = new Set(vendorModels.value.map((model) => model.modelName));
    currentVendorImportModels.value = response.data.models
      .filter((modelName: string) => !existingModelNames.has(modelName))
      .map((modelName: string) => ({ modelName, selected: true, type: "text" }));
    window.$message.success(`已拉取 ${currentVendorImportModels.value.length} 个可导入模型`);
  } catch (err: any) {
    window.$message.error(err.message ?? "模型列表拉取失败");
  } finally {
    currentVendorImportLoading.value = false;
  }
}

async function importCurrentVendorModels() {
  const vendor = currentVendor.value;
  const selectedModels = currentVendorImportModels.value.filter((model) => model.selected);
  if (!vendor || !selectedModels.length) return;

  currentVendorImportSaving.value = true;
  try {
    await Promise.all(
      selectedModels.map((item) =>
        axios.post("/setting/vendorConfig/addVendorModel", { id: vendor.id, model: createImportedCompatibleModel(item) }),
      ),
    );
    currentVendorImportVisible.value = false;
    await getVendorList();
    window.$message.success(`已导入 ${selectedModels.length} 个模型`);
  } catch (err: any) {
    window.$message.error(err.message ?? "模型导入失败");
  } finally {
    currentVendorImportSaving.value = false;
  }
}
...
async function fetchCompatibleModels() {
  ...
  try {
    const response = await axios.post("/setting/vendorConfig/fetchModels", {
      baseUrl: compatibleForm.value.baseUrl,
      apiKey: compatibleForm.value.apiKey,
    });
    compatibleModels.value = response.data.models.map((modelName: string) => ({ modelName, selected: true, type: "text" }));
    window.$message.success(`已拉取 ${compatibleModels.value.length} 个模型`);
  } catch (err: any) {
    window.$message.error(err.message ?? "模型列表拉取失败");
  } finally {
...
async function importCompatibleProvider() {
  const name = compatibleForm.value.name.trim();
  const selectedModels = compatibleModels.value.filter((model) => model.selected);
  if (!name || !selectedModels.length) return;

  compatibleSaving.value = true;
  const vendorId = createCompatibleVendorId(name);
  try {
    await axios.post("/setting/vendorConfig/addVendor", { tsCode: createCompatibleVendorCode(vendorId, name) });
    await Promise.all(
      selectedModels.map((item) => axios.post("/setting/vendorConfig/addVendorModel", { id: vendorId, model: createImportedCompatibleModel(item) })),
    );
    await getVendorList();
    ...
    window.$message.error(err.message ?? "供应商导入失败");
  } finally {
    compatibleSaving.value = false;
  }
}
