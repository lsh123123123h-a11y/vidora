...
import u from "@/utils";
import { assertMediaGenerationResult } from "@/lib/mediaGenerationResult";
import { retryImageGenerationRequest } from "@/lib/imageRequestRetry";
...
class AiImage {
  ...
  async run(input: ImageConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      ...
      this.result = assertMediaGenerationResult(await retryImageGenerationRequest(() => fn(input)));
