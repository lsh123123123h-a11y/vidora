import express from "express";
import u from "@/utils";
import { z } from "zod";
import { error, success } from "@/lib/responseFormat";
import { validateFields } from "@/middleware/middleware";
import axios from "axios";
import { assertRoleDerivativeModelSupport, resolveStoredRoleDerivativeGeneration } from "@/lib/roleDerivativeGenerationService";
const router = express.Router();

async function urlToBase64(imageUrl: string): Promise<string> {
  if (imageUrl.startsWith("/oss/")) {
    return await u.oss.getImageBase64(u.replaceUrl(imageUrl).replace("/smallImage", ""));
  }
  imageUrl = await u.oss.getFileUrl(u.replaceUrl(imageUrl));
  const response = await axios.get(imageUrl, { responseType: "arraybuffer" });
  const contentType = response.headers["content-type"] || "image/png";
  const base64 = Buffer.from(response.data, "binary").toString("base64");
  return `data:${contentType};base64,${base64}`;
}
export default router.post(
  "/",
  validateFields({
    model: z.string(),
    references: z.array(z.string()).optional(),
    quality: z.string(),
    ratio: z.string(),
    prompt: z.string(),
    projectId: z.number(),
    assetId: z.number().optional(),
  }),
  async (req, res) => {
    const { model, references = [], quality, ratio, prompt: requestedPrompt, projectId, assetId } = req.body;
    try {
      let prompt = requestedPrompt;
      let referenceList: { type: "image"; base64: string }[];
      let generationMode = "workflow";
      let parentAssetId: number | null = null;
      if (assetId != null) {
        const asset = await u
          .db("o_assets")
          .leftJoin("o_image", "o_assets.imageId", "o_image.id")
          .where({ "o_assets.id": assetId, "o_assets.projectId": projectId })
          .select("o_assets.type", "o_assets.assetsId", "o_assets.prompt", "o_image.filePath", "o_image.state")
          .first();
        if (!asset) throw new Error("The selected asset does not belong to this project.");

        if (asset.type === "role" && asset.assetsId != null) {
          const parent = await u
            .db("o_assets")
            .leftJoin("o_image", "o_assets.imageId", "o_image.id")
            .where({ "o_assets.id": asset.assetsId, "o_assets.projectId": projectId })
            .select("o_assets.type", "o_assets.assetsId", "o_assets.prompt", "o_image.filePath", "o_image.state")
            .first();
          await assertRoleDerivativeModelSupport(model);
          const derivative = await resolveStoredRoleDerivativeGeneration(parent, requestedPrompt);
          prompt = derivative.prompt;
          referenceList = derivative.referenceList;
          generationMode = "role-derivative";
          parentAssetId = asset.assetsId;
        } else {
          referenceList = [];
        }
      } else {
        referenceList = [];
      }

      if (!referenceList.length) {
        referenceList = [];
        for (const url of references) {
          referenceList.push({ type: "image", base64: await urlToBase64(url) });
        }
      }
      const imageClass = await u.Ai.Image(model).run(
        {
          prompt: prompt,
          referenceList,
          size: quality,
          aspectRatio: ratio,
        },
        {
          taskClass: "工作流图片生成",
          describe: "工作流图片生成",
          relatedObjects: JSON.stringify({ assetId: assetId ?? null, parentAssetId, generationMode, referenceImageCount: referenceList.length }),
          projectId: projectId,
        },
      );
      const savePath = `${projectId}/workFlow/${u.uuid()}.jpg`;
      await imageClass.save(savePath);

      const url = await u.oss.getSmallImageUrl(savePath);
      return res.status(200).send(success({ url }));
    } catch (e) {
      res.status(400).send(error(u.error(e).message));
    }
  },
);
