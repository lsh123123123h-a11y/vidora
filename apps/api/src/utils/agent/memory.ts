...
class Memory {
  private static readonly summaryLocks = new Set<string>();
  ...
  constructor(agentType: string, isolationKey: string) {
    ...
    this.modelKey = agentType === "scriptAgent" || agentType === "productionAgent" ? `${agentType}:decisionAgent` : agentType;
  ...
  private async generateSummary(contents: string[]): Promise<string> {
    ...
    const { text } = await u.Ai.Text(this.modelKey as any).invoke({
  ...
  private scheduleSummary() {
    if (Memory.summaryLocks.has(this.isolationKey)) return;

    Memory.summaryLocks.add(this.isolationKey);
    void this.createPendingSummaries()
      .catch((error) => console.error(`[Memory] Failed to generate summaries for ${this.isolationKey}`, error))
      .finally(() => Memory.summaryLocks.delete(this.isolationKey));
  }

  private async createPendingSummaries() {
    ...
    while (true) {
      const unsummarized = await u.db("memories").where({ isolationKey: this.isolationKey, type: "message", summarized: 0 }).orderBy("createTime", "asc");
      if (unsummarized.length < batchSize) return;
      ...
      await u.db.transaction(async (trx) => {
        const claimed = await (trx("memories") as any).whereIn("id", batchIds).where("summarized", 0).update({ summarized: 1 });
        if (claimed !== batchIds.length) return;
