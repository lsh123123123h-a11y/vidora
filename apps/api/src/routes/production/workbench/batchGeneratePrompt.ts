...
import { buildVideoPromptContextFromSources } from "@/lib/videoPromptContextService";
    ...
    try {
      ...
      const tasks = trackData.map((track: { trackId: number; info: { id: number; sources: string }[] }) =>
        limit(async () => {
          ...
          try {
            const content = await buildVideoPromptContextFromSources(projectId, modelData, track.info);
