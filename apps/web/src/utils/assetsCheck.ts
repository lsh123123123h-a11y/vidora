...
export default function openAssetsSelector(options: AssetsSelectOptions = {}): Promise<Asset[]> {
  const { types, clipMediaTypes, multiple = true, title = window.$t("common.selectAssets"), selectorMode = false, excludeRootRoles = false } = options;
      ...
      const vnode = h(
        TDialog,
        {
          ...
          onConfirm: () => {
            ...
            // 选中的父资产
            const selectedParents = data.filter((item) => selectedKeys.includes(item.id) && !(excludeRootRoles && item.type === "role" && item.assetsId == null));
            // 选中的子资产
