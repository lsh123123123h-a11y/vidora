...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  const response = await fetch(`${baseUrl}/images/generations`, {
    ...
    body: JSON.stringify({
      ...
      ...(config.referenceList?.length ? { images: config.referenceList.map((item) => item.base64) } : {}),
    }),
  });
  if (!response.ok) {
    const detail = await response.text();
    throw new Error(`图片生成请求失败: HTTP ${response.status}${detail ? ` ${detail.slice(0, 500)}` : ""}`);
  }
  const payload = await response.json();
  const image = payload?.data?.[0];
  if (typeof image?.b64_json === "string" && image.b64_json.trim()) return image.b64_json.trim();
 ...
 *
 * 【概述】
 * 本文件是 Vidora AI 供应商适配模板。AI 在生成新供应商适配代码时，
