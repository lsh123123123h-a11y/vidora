...
import { resolveSupportedVideoSettings } from "@/utils/videoGenerationPolicy";
...
const availableResolutions = computed(() => [
  ...new Set((props.modeOptions.durationResolutionMap ?? []).flatMap((option) => option.resolution ?? [])),
]);
const availableDurations = computed(() => [
  ...new Set(
    (props.modeOptions.durationResolutionMap ?? [])
      .filter((option) => option.resolution?.includes(modelParmas.value.resolution))
      .flatMap((option) => option.duration ?? []),
  ),
].sort((a, b) => a - b));
function updateResolution(resolution: string) {
  const settings = resolveSupportedVideoSettings(props.modeOptions.durationResolutionMap ?? [], modelParmas.value.duration, resolution);
