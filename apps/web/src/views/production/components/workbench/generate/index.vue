...
function clampDuration(trackDuration: number): number {
  const result = resolveSupportedVideoSettings(modeOptions.value?.durationResolutionMap ?? [], trackDuration, modelParmas.value.resolution);
  if (result.resolution !== modelParmas.value.resolution) modelParmas.value.resolution = result.resolution;
    ...
    onConfirm: async () => {
      ...
      try {
        const { data } = await axios.post("/production/workbench/generateVideo", {
          ...
          duration: clampDuration(currentTrack.value.duration || modelParmas.value.duration),
