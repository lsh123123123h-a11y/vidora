...
import { buildRoleDerivativePrompt } from "@/lib/characterGenerationPolicy";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const assetsDataArr = await u.db("o_assets").where({ projectId }).whereIn("id", assetIds).select("id", "describe", "name", "type", "assetsId");
    if (assetsDataArr.length !== new Set(assetIds).size) return res.status(400).send(error("Some requested assets do not belong to this project."));
    ...
    const parentAssetsData = await u
      ...
      .select("o_assets.id", "o_assets.type", "o_assets.assetsId", "o_image.filePath", "o_image.state", "o_assets.describe", "o_assets.prompt");
    assetsDataArr.forEach((i: any) => {
      ...
      if (parent) {
        ...
        i.parentState = parent.state;
      }
    });
    if (assetsDataArr.some((item) => item.type === "role" && item.assetsId != null)) {
      await assertRoleDerivativeModelSupport(projectSettingData?.imageModel || "");
    }
    const forcedRoleReferenceByAssetId = new Map<number, string>();
    for (const item of assetsDataArr) {
      if (item.type !== "role" || item.assetsId == null) continue;
      const derivative = await resolveStoredRoleDerivativeGeneration(
        parentAssetsData.find((candidate) => candidate.id === item.assetsId),
        item.describe || "",
      );
      forcedRoleReferenceByAssetId.set(item.id!, derivative.referenceList[0].base64);
    }
    ...
    const generateSingleAsset = async (item: any) => {
        ...
        messages: [
          {
            role: "user",
            content: `
            Parent role identity prompt: ${item.parentPrompt || "No parent identity prompt is available."}
      ...
      let imageBase64 = imageUrlRecord[item.assetsId!] ? await u.oss.getImageBase64(imageUrlRecord[item.assetsId!]) : null;
      if (item.type === "role" && item.assetsId != null) {
        const parent = parentAssetsData.find((candidate) => candidate.id === item.assetsId);
        promptForImage = buildRoleDerivativePrompt(parent?.prompt, text);
        imageBase64 = forcedRoleReferenceByAssetId.get(item.id) || null;
      }
      try {
        const repeloadObj = {
        ...
        const imageCls = await u.Ai.Image(projectSettingData?.imageModel as `${string}:${string}`).run(
          ...
          },
          {
            ...
            relatedObjects: JSON.stringify({
              ...repeloadObj,
              generationMode: item.type === "role" && item.assetsId != null ? "role-derivative" : "asset",
              parentAssetId: item.assetsId ?? null,
              referenceImageCount: imageBase64 ? 1 : 0,
            }),
            projectId: projectId,
          },
        );
