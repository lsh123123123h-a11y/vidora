...
import { masterRoleConstraints } from "@/lib/characterGenerationPolicy";
  ...
  async (req, res) => {
    ...
    const tasks = items.map((item: { assetsId: number; type: string; name: string; describe: string }) =>
      limit(async () => {
        ...
        const systemPrompt = item.type === "role" && !assetData.assetsId ? `${visualManual}\n${masterRoleConstraints}` : visualManual;
        try {
