...
import { withAgentRequestTimeout } from "@/utils/agent/requestTimeout";
...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const { fullStream } = await u.Ai.Text("scriptAgent:decisionAgent", ctx.thinkConfig.think, ctx.thinkConfig.thinlLevel).stream({
    ...
    abortSignal: withAgentRequestTimeout(abortSignal),
    tools: {
      ...memory.getTools(),
...
function createSubAgent(parentCtx: AgentContext) {
  ...
  const run_sub_agent_storySkeleton = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const formatPrompt =
        "\nYou must call set_planData with key storySkeleton to save the completed story skeleton. Then output <storySkeleton>content</storySkeleton> so the workspace updates immediately.";
  ...
  const run_sub_agent_adaptationStrategy = tool({
    ...
    execute: async ({ prompt }) => {
      ...
      const formatPrompt =
        "\nYou must call set_planData with key adaptationStrategy to save the completed adaptation strategy. Then output <adaptationStrategy>content</adaptationStrategy> so the workspace updates immediately.";
