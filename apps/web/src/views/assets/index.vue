...
function handleSelectChange(value: Array<string | number>) {
  const filtered = value.filter((key) => {
    if (isGenerating(key as number)) return false;
    const asset = tableData.value.find((item: any) => item.id === key);
    return !(props.excludeRootRoles && asset?.type === "role" && asset.assetsId == null);
  });
  if (!props.multiple) {
