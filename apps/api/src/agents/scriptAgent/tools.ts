...
const writablePlanDataKeySchema = z.enum(["storySkeleton", "adaptationStrategy"]);
...
export function mergePlanData(current: Record<string, unknown>, key: "storySkeleton" | "adaptationStrategy", value: string) {
...
export default (toolCpnfig: ToolConfig) => {
  ...
  const tools: Record<string, Tool> = {
    ...
    set_planData: tool({
      description: "Save story skeleton or adaptation strategy to the project workspace.",
      inputSchema: jsonSchema<{ key: "storySkeleton" | "adaptationStrategy"; value: string }>(
        z
          .object({
            key: writablePlanDataKeySchema,
      ...
      execute: async ({ key, value }) => {
        ...
        const data = mergePlanData(current, key, value);

        if (row) {
          await u.db("o_agentWorkData").where({ id: row.id }).update({ data: JSON.stringify(data) });
        } else {
          await u.db("o_agentWorkData").insert({ projectId, key: "scriptAgent", data: JSON.stringify(data) });
        }

        socket.emit("setPlanData", { key, value });
        return `${planDataKeyLabels[key]} saved successfully.`;
      },
    }),
    get_novel_text: tool({
